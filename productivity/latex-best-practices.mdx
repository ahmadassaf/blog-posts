...
- I cannot stress enough the importance of using a versioning system like [`git`](http://git-scm.com/) or [`svn`](https://subversion.apache.org/). It is vital for collaborative authoring.
...
- BibTeX looks in the `.aux` file for the list of references, goes to the `.bib` file, extracts the relevant entries, and puts them in a local `.bbl` file ready for insertion into the document for each paper.
...
- [refcheck](https://www.ctan.org/tex-archive/macros/latex/contrib/refcheck) for checking references in a document: numbered but unlabeled equations, labels that are never used in the text, and unused bibliography entries.
