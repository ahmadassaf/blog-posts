...
The Linked Data publishing best practices [@Bizer:DB:11] specifies that datasets should contain metadata needed to effectively understand and use them. *Metadata* is structured information that describes, explains, locates, or otherwise makes it easier to retrieve, use, or manage an information resource [@NISO:techReport:04]. Having rich metadata helps in enabling:
 ...
 - **Organization and identification**: The increasing number of datasets being published makes it hard to track and organize them, bring similar resources together and distinguish useful links.
...
There are many data portals that host a large number of private and public datasets. Each portal presents the data based on a model used by the underlying software. In this section, we present the results of our landscape survey of the most common data portals and dataset models.
...
[CKAN](http://ckan.org) was, at the time of this survey in 2015, the world's leading open-source data management system (DMS), and it remains widely deployed. It helps users from different domains (national and regional governments, companies and organizations) to easily publish their data through a set of workflows to publish, share, search and manage datasets. CKAN is the portal powering web sites like Datahub, the Europe's Public Data portal or the [U.S Government's open data portal](http://data.gov).
...
[Socrata](https://support.socrata.com/) is a commercial platform to streamline data publishing, management, analysis and reusing. It empowers users to review, compare, visualize and analyze data in real time. Datasets hosted in Socrata can be accessed using RESTful API that facilitates search and data filtering.
...
A dataset metadata model should contain sufficient information so that consumers can easily understand and process the data that is described. After analyzing the models described in the previous section, we find out that a dataset can contain four main sections:
...
-   **Access information**: Information about dataset access and usage (e.g., URL, license title and license URL). In addition to the properties in the models discussed above, there are several vocabularies designed specially to describe data access right e.g. [Linked Data Rights](https://lov.linkeddata.es/dataset/lov/vocabs/ldr), the [Open Digital Rights Language (ODRL)](http://www.w3.org/ns/odrl/2/).
...
-   **Provenance information**: Temporal and historical information about the dataset creation and update records, in addition to versioning information (e.g. creation data, metadata update data, latest version). Provenance information coverage varies across the models surveyed. However, its great importance led to the development of various special vocabularies like the [Open Provenance Model](http://open-biomed.sourceforge.net/opmv/) and PROV-O [@Lebo:W3C:13]. DataID [@Brummer::ICSS:14] is an effort to provide semantically rich metadata with focus on providing detailed provenance, license and access information.
...
-   **Statistical information**: Statistical information about the data types and patterns in datasets (e.g. properties distribution, number of entities and RDF triples). This information is particularly useful to explore a dataset as it gives detailed insights about the raw data when provided properly. VoID is the only model that provides statistical information about a dataset. VoID defines properties to express different statistical characteristics of a dataset e.g. the total number of triples, entities or distinct classes. However, there are other vocabularies such as SCOVO [@Hausenblas:ESWC:09] that can model and publish statistical data about datasets.

-   **Quality information**: Information that indicates the quality of the dataset on the metadata and instance levels. In addition to that, a dataset should include an openness score that measures its alignment with the Linked Data publishing standards [@Berners-Lee:W3C:06]. Quality information is only expressed in the POD metadata. However, there are various other vocabularies like daQ [@Debattista:WWW:14] that can be used to express datasets quality. The [RDF Review Vocabulary](http://vocab.org/review/) can also be used to express reviews and ratings about the dataset or its resources. I later built on this observation with an [objective assessment framework for Linked Data quality](/blog/category/data/an-objective-assessment-framework-tool-for-linked-data-quality) that scores such metadata automatically.

## Towards A Harmonized Model

Since establishing a common vocabulary or model is the key to communication, we identified the need for a harmonized dataset metadata model containing sufficient information so that consumers can easily understand and process datasets. To create the mappings between the different models, we performed various steps:
...
The table above shows only these section-level mappings; the full mappings between the models across all the information groups are given in the paper, linked again at the end of this section (there, entries in the CKAN column marked with $\ast$ are properties from CKAN extensions and not included in the original data model). Similar to the sections mappings, for the ontologies (DCAT, VoID), the first part represents the class and the part after $\rightarrow$ represents the property. However, sometimes the part after $\rightarrow$ refers to another resource. For example, to describe the dataset's maintainer email in DCAT, the information should be presented in the `dcat:Dataset` class using the `dcat:contactPoint` property. However, the range of this property is a resource of type `vcard` which has the property `hasEmail`.
...
Examining the different models, we noticed a lack of a complete model that covers all the information types. There is an abundance of extensions and application profiles that try to fill in those gaps, but they are usually domain specific addressing specific issues like geographic or temporal information. At the time of the survey, there was no complete model that encompassed all the described information types. In hindsight, the standards eventually caught up: DCAT 2 (2020) and DCAT 3 (2024) later addressed much of the granularity gap this paper identified, including richer distribution, provenance and dataset series descriptions.

HDL aims at filling this gap by taking the best from these models. HDL is currently modeled in [JSON](https://github.com/ahmadassaf/roomba/blob/master/model/hdl.json) but converting it to a standalone OWL ontology is part of our future work. To give a feel for its shape, here is an abridged excerpt of the model: general, access, ownership and provenance fields at the dataset level, with nested objects for the maintainer, author, organizations, resources (including temporal and spatial coverage), groups and tags:

```json
{
    "id": "", "name": "", "title": "", "description": "",
    "url": "", "type": "", "accessLevel": "", "language": "",
    "licenseTitle": "", "licenseID": "", "licenseUrl": "",
    "version": "", "metadataCreated": "", "metadataModified": "",
    "maintainer": { "name": "", "email": "", "role": "" },
    "author": { "name": "", "email": "", "role": "" },
    "organization": [{ "id": "", "name": "", "description": "", "location": "" }],
    "resources": [{
        "name": "", "downloadUrl": "", "accessUrl": "", "format": "",
        "mimetype": "", "size": "", "dataQuality": "",
        "temporal": { "temporalGranularity": "", "temporalCoverageFrom": "", "temporalCoverageTo": "" },
        "spatial": { "spatialText": "", "spatialGranularity": "", "bbox": "" }
    }],
    "groups": [{ "id": "", "name": "", "title": "", "subGroupOf": "" }],
    "tags": [{ "id": "", "name": "", "vocabularyId": "" }]
}
```

The full model, together with the complete mapping table across the seven surveyed models, lives in the [Roomba repository](https://github.com/ahmadassaf/roomba/blob/master/model/hdl.json) and in the [paper](https://ceur-ws.org/Vol-1362/PROFILES2015_paper3.pdf).

The CKAN model controls the values to be used in describing some dataset properties. For example, the `resource_type` property can have the values: file: direct accessible bitstream, file.upload: file uploaded to the [CKAN FileStore](https://docs.ckan.org/en/latest/maintaining/filestore.html), api, visualization, code: the actual source code or a reference to a code repository and documentation. However, using the Roomba tool [@Assaf:WWW:15], we managed to generate portal-wide reports about the representation of various fields in CKAN portals. The goal behind these reports is to find what are the frequent fields data publishers are adding as `extras` fields.
