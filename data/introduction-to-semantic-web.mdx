...
Before the Web, if I wanted to follow a citation I had to physically hunt down the reference in a library. The Web's great invention, the hyperlink, changed that: click a link and you are at the next document in the chain, which is why **Web 1.0** was the web of read-only documents. **Web 2.0** brought interactive, social applications, but they were silos that do not interoperate; updating your profile on one social network does nothing to your profile on another, and the same lack of linking plagues data inside enterprises. **[Web 3.0](http://www.w3.org/standards/semanticweb)** is about connecting the data itself, <Highlight>not the documents but the data at lower levels</Highlight>, to facilitate machine-to-machine communication.
...
The Semantic Web addresses the discoverability challenges through the adoption of distinct identifiers for concepts and their relationships. These identifiers, referred to as Universal Resource Identifiers (URIs), resemble web page URLs but are not restricted to identifying web documents. Instead, their primary purpose is to provide unique identification for objects and concepts, as well as their interconnections.
...
- **Query Language**: [SPARQL](/blog/category/data/sparql-a-comprehensive-guide): The query language of the Semantic Web. It is specifically designed to query data across various systems.
- **Schema and Ontology Languages**: [Web Ontology Language (OWL)](http://en.wikipedia.org/wiki/Web_Ontology_Language 'Web Ontology Language') and [RDF Schema (RDFS)](https://en.wikipedia.org/wiki/RDF_Schema): The schema language, or knowledge representation (KR) language, of the Semantic Web. They enable you to define concepts in a composable way so that these concepts can be reused as much and as often as possible. Composability means that each concept is carefully defined to be selected and assembled in various combinations with other concepts as needed for many different applications and purposes.
...
To make these rules concrete, imagine two tables living in two separate databases: a customers table in a CRM system and an orders table in a billing system. Each order row points at a customer, but only through an internal key that means nothing outside its own database. Applying the four rules to those two tables looks like this:
...
4. Once all the rows of our tables have been uniquely identified, made dereferenceable through HTTP, and described with RDF, the last step is providing links between different rows across different tables. This is the step where the implicit links become explicit: it is what makes the data Linked Data.
...
Searching websites using keywords has been a long-standing method for finding information online, but relying solely on words to determine the content of a page is limiting: it restricts the potential use of the information on your page, and we're just beginning to fully leverage the vast amount of knowledge available on the global web. New standards for Structured Data help inform search engines about the actual entities a webpage contains, beyond just keywords, transitioning from a web of keywords to a more powerful web of things. This structured data on web pages embodies Tim Berners-Lee's vision of the semantic web, allowing machines, not just humans, to understand web knowledge.
...
I find these ideas more relevant than ever in the era of large language models. LLMs are fluent but they hallucinate, and knowledge graphs give us a way to ground their answers in explicit, verifiable facts. The machine-readable, structured data this post describes is precisely what feeds modern retrieval systems, so the Semantic Web's core bet, that meaning should be machine-readable, still holds up.
