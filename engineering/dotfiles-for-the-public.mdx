...
*This post is from 2015. The concepts hold up well, but many of the specific tools and apps named here have since died or been replaced, and my own setup has moved on to [gaudi-bash](/blog/category/engineering/gaudi-my-bash-framework).*
...
This file speeds up the process of running executable files. Rather than having to cd back and forth across various paths to executable files, you can set the file paths in your .path dotfile and then run executable files directly.
...
Sets environment variables, such as setting Sublime Text as the default editor using export `EDITOR="sublime"` It also increases the amount of history saved, useful for backtracking over previous commands you've used.
...
Before, when I mentioned I was looking over different dotfile repos, I did `mkdir` to create a directory. After that, I'd then need to `cd` into that directory. A function collapses those two steps into one command:
...
Once patterns like this live in your `.functions` file, they follow you to every machine.
...
Similar to .gitignore for Mercurial.
...
- A dependency is the [powerline fonts](https://github.com/powerline/fonts) pack. Installation instructions can be found directly in the repository.
...
This can be changed **depending on the path to the `python` directory**
...
Some of the files mentioned above don't live in my dotfiles repo itself. That is because I am using my fork of the amazing [bash-it](http://github.com/ahmadassaf/bash-it) framework to organize those dotfiles. I recommend you head over there and read how to use them.
...
When setting up a new Mac, you may want to install some common [Homebrew](http://brew.sh/) formulae (after installing Homebrew, of course). At the time I wrote this, `brew bundle` had been deprecated, so I converted my Brewfiles into plain executable shell scripts that call `brew install` line by line:

```bash
./.brewfile.sh
```

Homebrew has since brought `brew bundle` back, and today a declarative `Brewfile` driven by `brew bundle` is the standard way to do this. If you are setting this up now, use that instead.

You can always remove or add formulae based on your preferences. You can search for brew formulas [here](https://formulae.brew.sh/) in order to add or get more information about the ones I am using.
...
You could also install native apps with [`brew cask`](https://github.com/phinze/homebrew-cask). Same story as the Brewfile above, my cask list lived in an executable script:

```bash
./.caskfile.sh
```

You can always remove or add apps based on your preferences. You can search for cask formulas [here](https://brew.sh/) in order to add or get more information about the ones I am using.
...
- [**psi**](https://github.com/GoogleChromeLabs/psi): PageSpeed Insights with reporting
...
- [Awesome dotfiles tutorials](https://github.com/webpro/awesome-dotfiles#tutorials)
