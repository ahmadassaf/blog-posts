...
*This post is from 2016. Travis CI's free tier for open source is long gone, and this exact setup would live in GitHub Actions today, but the pattern (build, validate, push the built site to the server over SSH) holds up unchanged.*
...
Jekyll website has nice [articles on continuous integration](https://jekyllrb.com/docs/deployment/automated/) and [Deployment](https://jekyllrb.com/docs/deployment-methods/). In a nutshell, serving Jekyll can be done in two ways:
...
Whereas you substitute the location of your github repo in the server to point correctly, e.g., `/www/blog`.
...
I have tried to add a remote in my web server called `production` so that I can push directly to my server with `git push production master`: the data is pushed indeed, but I still don't get the hooks to be run at all.

As a result of my failed attempts, I had to try another approach and I decided to go and try [TravisCI](https://www.travis-ci.com).
...
<PipelineDiagram
  ...
  description='A push to GitHub triggers a build, validation, and deployment pipeline. Failed stages stop early; a successful deployment publishes the site and returns its link to GitHub.'
...
Enter file in which to save the key (/Users/USER_NAME/.ssh/id_rsa): deploy-key
```
