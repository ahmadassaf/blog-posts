...
*This post is from 2016, when this blog ran on Jekyll. The versions, plugins, and build tooling described here are long out of date, and the blog has since [moved to Next.js](/blog/category/engineering/moving-blog-to-nextjs). I keep it as a record of that setup.*
...
Jekyll is a command-line executable built with Ruby and has a few commands we need to run from time to time. Ruby came installed by default on various operating systems; at the time of writing, the latest stable version was 2.3.1, and anything above 2.2 was fine. For upgrades I used rbenv to make it easy.

Installing the then-current Jekyll was a matter of executing:
...
Jekyll comes with a built-in development server. The `jekyll serve` command starts this server on your machine and starts watching your files for changes similar to Grunt or Gulp.
...
- **\_posts**: The main folder that contains the markdown posts
...
## Files I found worth knowing about
...
- **disquss.shortname**: This is the Disqus website shortname which is used to inject `_includes/partials/post/comments.html`
...
What is notable about a **page** is the ability to define a `permalink` in the front matter e.g., `permalink: /tags.html`. This means that this is the page that will be accessed from `{{site.url}}/tags`. Jekyll will then look at the `layout` defined in the front matter and render the correct file from `_layouts`.

## Collaborating on Posts

One of the reasons I moved to a Jekyll, markdown-based blog is to enable people to contribute to my posts. To do so, I host the posts into a [separate Github repository](https://github.com/ahmadassaf/blog-posts) and include that as a separate git submodule by configuring the `.gitmodules` as follows:

```
[submodule "_posts"]
    path = _posts
    url = https://github.com/ahmadassaf/blog-posts.git
```
...
- Open Graph, Twitter and Schema.org annotation enabled
...
Note that the directory we are referring to is `/images/posts`. This is the directory that we will copy all the posts images to using our grunt build script. Also you can pass multiple images in between the `capture`
...
- [Pygments styles gallery](https://pygments.org/styles/)
...
One thing I wanted to add is the ability to perform full text search on the posts. To do so, I first wanted to be able to grab the posts data and have them saved. I managed to do that by creating the `/posts.json`:
...
Now, I would like to be able to easily deploy my application on my live server as well as being able to run and test it in my local machine. The main thing to note here is that we need to have the `url` pointing to two different URIs on each machine. To do this dynamically, I created a new `_config.dev.yml` file that will overwrite only the `url` setting to run on `localhost:4000` and kept the url in my main `_config.yml` to my live server address.
...
> note that the `--incremental` is a default optional flag to optimize the build and serve process by rebuilding changed files only and not the whole site

## Using Jekyll, Grunt and Browserify

I use [Browserify](http://browserify.org/) in my `main.js` to easily bundle up all of my front-end dependencies. Now, if I am serving my site and want to test my JavaScript changes, it is painful to rebuild the site every time for that. The jekyll watch feature will not run browserify for me, so instead I use the [grunt-concurrent](https://github.com/sindresorhus/grunt-concurrent) with a `watch` task defined to monitor and fire a browserify build whenever a change is detected and a background shell process using [grunt-bg-shell](https://github.com/rma4ok/grunt-bg-shell) to serve. They all come together like:
