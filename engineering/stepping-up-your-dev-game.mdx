...
*This post is from 2015, the macOS Yosemite era. Nearly every tool named below has since died, been acquired, or been broken by later macOS releases; I keep the post as a record of what a carefully tuned Mac setup looked like at the time. The dotfiles philosophy is the only part that aged well.*

Every developer eventually faces the moment of setting up a new working environment. For me, the transition from a Windows powered PC to a Mac OSX powered laptop was one of the best things I did in my career. I found the openness of things you can do on a Unix based system accompanied with a solid appealing set of software.
However, setting everything the way you **want** it can be time consuming, so I decided after a few trials and errors to share my experience in customizing my machine and how it can be easily transferred into other machines as well.

<Callout type='info'>For up-to-date instructions on how to set up your machine and deal with dotfiles, head over to my article about my bash framework [**gaudi**](/blog/category/engineering/gaudi-my-bash-framework)</Callout>

## Customizing the UI

First, let's start with the lightweight things. The desktop and general UI enhancement. I always loved to show some nice widgets on my desktop, and for that I used [Übersicht](http://tracesof.net/uebersicht/). Before it I had used [Geektool](http://projects.tynsoe.org/en/geektool/), which basically allows you to run custom scripts and display them on the desktop, but I wasn't at all happy with the experience of styling those "widgets" and the availability of 3rd party widgets.
...
There was already a nice set of widgets published in the [gallery](http://tracesof.net/uebersicht-widgets/). In my `widgets` folder, you will find the set of widgets I was using with customized positioning and styling.
...
- [weather](https://github.com/ahmadassaf/configurations/tree/master/widgets/weather.widget) to show the nice weather forecast. It used the Free Geo IP API to obtain your location, and the Yahoo Weather API (both services are gone now) to obtain the weather information.
- **System Stats**
  ...
  - [public_ip](https://github.com/ahmadassaf/configurations/tree/master/widgets/public_ip.widget): Displays the current public IP address.
...
- Installing Übersicht could be done using the downloadable zip file, or the recommended way would be via cask (which I will explain later) via `brew cask install ubersicht`
- Installing the widgets is done by copying the **contents** of the widgets folder into `Library -> Application Support -> Übersicht -> widgets`. However, what I did was set up my widgets to be in my Github repo or dropbox folder so that they are synced and can be backed up, and I linked the folder directly to the desired location via `ln -s ~/Projects/Configurations/config/widgets/ ~/Library/Application\ Support/Übersicht/widgets` where the first parameter is the location of the `widgets/` folder cloned.
  **Note** I have included the folder link in my `.osx` file, you may need to adjust the path of the first argument.

## Other Widgets

One element that I used that is not part of Übersicht is Dateline, a discrete desktop calendar with support for iCal and other calendar applications (it has since disappeared from sale).
...
In the `flat-icons` you will find the set of flat icons I used for my mounted HDD icons and the dock.
To easily change application icons, I used LiteIcon, which could also be installed via my `caskfile.sh`.

To further customize the dock, I used Dockmod, which was the most feature-rich application for dock theming with support up to OSX Yosemite, in order to provide a transparent Dock background. That whole category of tooling is dead: System Integrity Protection, introduced right after Yosemite in El Capitan, put an end to dock theming of this kind.

I also removed the arrow icons for aliases/shortcuts, which was done automatically when executing my `.osx` file or manually by typing the following command in the terminal (another thing SIP no longer allows):
...
Although I had the latest OSX Yosemite update, I was still unhappy with the capabilities of the Finder. I found that [PathFinder](https://cocoatech.io/) was a great replacement. Just head over and you can check the list of things you can do with this great piece of software.
...
- **[Caffeine](https://www.caffeine-app.net/)**: Caffeine is a tiny program that puts an icon in the right side of your menu bar. Click it to prevent your Mac from automatically going to sleep, dimming the screen or starting screen savers.
- **Flashlight**: an unofficial Spotlight API that allowed you to programmatically process queries and add additional results. It was a rough but fun proof of concept; it is long dead, since macOS never opened Spotlight up officially.
...
The Workflows I used are (Located in the `workflows` folder):
...
My configuration consisted of the following:

- Mac OSX terminal with bash shell configured with a forked [bash-it](http://github.com/ahmadassaf/bash-it) configurations
- iTerm2 terminal with zsh shell configured with [oh-my-zsh](https://github.com/ohmyzsh/ohmyzsh)
...
First of all you will have to clone this repository into your local machine, and since there are a bunch of other git modules in this repo, you will need to recursively clone this repository and all of its children using:
...
- It will install `homebrew` and the following brew formulas:
  ...
  - **Git specific addons** git-extras, hub, bfg
  ...
  - **[Core (formerly Dupes)](https://github.com/Homebrew/homebrew-core)** grep, screen

- It will install `cask` and the following applications (reading this list back is a small graveyard tour: Mailbox was shut down by Dropbox in 2016, Screenhero was absorbed into Slack and discontinued, and Dockmod and LiteIcon did not survive SIP):
  ...
  - **Color pickers** colorpicker-developer, colorpicker-skalacolor

- It will aggregate the `iTerm` and `Terminal` themes and prompt you for the name of the color scheme you want to apply and apply it for you
...
- It will install the [powerline fonts](https://github.com/powerline/fonts)
